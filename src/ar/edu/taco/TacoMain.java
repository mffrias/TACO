...
public class TacoMain{
	...
	public TacoAnalysisResult run(String configFile, Properties overridingProperties) throws IllegalArgumentException {
		...
		while (!pendingProblems.isEmpty() || !problemsToFurtherDeterminize.isEmpty() || !theSharedQueue.isEmpty() || theRunningThreads > 0) {
			...
			if (!pendingProblems.isEmpty() && someFreeThread) {
				...
				determinizedWrapped.setTimeout(timeout);
			...
			if (System.currentTimeMillis() >= previousTime + 1000) {
				...
				numErrors = numAttended - numSAT - numUNSAT - numInterrupted - theRunningThreads;
				...
				title = makeTitle("Time ellapsed", "Num Attended", "Num SAT", "Num UNSAT", "Num Errors", "Num Interrupted", "Num Pending", "Num To Split", "Num Running Threads");
				content = makeContent(currentTime / 1000, numAttended, numSAT, numUNSAT, numErrors, numInterrupted, numPending, toSplitSize, theRunningThreads);
	...
	private String makeContent(long elapsedTime, int numAttended, int numSAT, int numUNSAT, int numErrors,
			int numInterrupted, int numPending, int toSplitSize, int runningThreads) {
		// TODO Auto-generated method stub
		String theSpace = "";
		String theContent = String.format("%2$12d %1$4s %3$12d %1$4s %4$7d %1$4s %5$9d %1$4s %6$10d %1$4s %7$15d %1$4s %8$11d %1$4s %9$12d %1$4s %10$19d", theSpace, elapsedTime, numAttended, numSAT, numUNSAT, numErrors, numInterrupted, numPending, toSplitSize, runningThreads);

		return theContent;
	}

	private String makeTitle(String elapsed, String attended, String numSAT, String numUNSAT, String numErrors,
			String numInterrupted, String numPending, String toSplitSize, String numRunningThreads) {
		// TODO Auto-generated method stub
		...
		String theSpace = "";
		String theTitle = String.format("%2$12s %1$4s %3$12s %1$4s %4$7s %1$4s %5$9s %1$4s %6$10s %1$4s %7$15s %1$4s %8$11s %1$4s %9$12s %1$4s %10$19s", theSpace, elapsedTitle, numAttendedTitle, NumSATTitle, NumUNSATTitle, numErrorsTitle, numInterruptedTitle, numPendingTitle, numToSplitTitle, numRunnThreadsTitle);
		return theTitle;
	}
	...
	public ConcurrentLinkedQueue<JCompilationUnitTypeWrapper> removeNonDeterminism(JCompilationUnitTypeWrapper simpleUnit, int size) {
