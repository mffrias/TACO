...
//		if (function_ids.contains("fun_java_primitive_integer_value_size_of") || predicate_ids.contains("pred_java_primitive_integer_value_size_of")) {
    ...
    private Scope infer_program_scope() {
        ...
        InputCount input_count = countAllInputs();

        ContractOpsCount countContractOps = countAllContractOps();
        ...
        inferred_program_scope.setInputScopeInteger(java_primitive_integer_value_sig_id, inferred_integer_program_scope + input_count.integer_input_count + countContractOps.integer_op_contracts_count);
        ...
        inferred_program_scope.setInputScopeInteger(java_primitive_long_value_sig_id, inferred_long_program_scope + input_count.long_input_count + countContractOps.long_op_contracts_count);
        ...
        inferred_program_scope.setInputScopeInteger(java_primitive_float_value_sig_id, inferred_float_program_scope + input_count.float_input_count + countContractOps.float_op_contracts_count);
        ...
        inferred_program_scope.setInputScopeInteger(java_primitive_char_value_sig_id, inferred_char_program_scope + input_count.char_input_count + countContractOps.char_op_contracts_count);
    ...
    private int infer_integer_program_scope(int integer_literal_count, IntegerOpCounter integer_op_counter) {
        ...
        int inferred_sizeof_applications_scope = integer_op_counter.num_of_sizeof();
        ...
        inferred_integer_program_scope += inferred_sizeof_applications_scope;
    ...
    private static class InputCount {
        int float_input_count = 0;
        int integer_input_count = 0;
        int long_input_count = 0;
        int char_input_count = 0;
    }

    private static class ContractOpsCount {
        int float_op_contracts_count = 0;
        int integer_op_contracts_count = 0;
        int long_op_contracts_count = 0;
        int char_op_contracts_count = 0;
    }
    ...
    private ContractOpsCount countAllContractOps() {
        ContractOpsCount contractOpsCount = null;
        for (JDynAlloyModule module : this.src_jdynalloy_modules) {
            if (module.getModuleId().equals(TacoConfigurator.getInstance().getClassToCheck())) {
                Set<JObjectInvariant> theInvariants = module.getObjectInvariants();
                contractOpsCount = this.countAllContractOpsInModule(module);
                return contractOpsCount;
            }
        }
        return null;
    }

    private ContractOpsCount countAllContractOpsInModule(JDynAlloyModule module) {
        ContractOpsCount theScope = new ContractOpsCount();
        List<AlloyFormula> thePreds = module.getPredsEncodingValueOfArithmeticOperationsInObjectInvariants();
        for (AlloyFormula pred : thePreds){
            if (pred.toString().contains("java_primitive_integer_value")){
                theScope.integer_op_contracts_count++;
            } else {
                if (pred.toString().contains("java_primitive_float_value")) {
                    theScope.float_op_contracts_count++;
                } else {
                    if (pred.toString().contains("java_primitive_float_value")) {
                        theScope.long_op_contracts_count++;
                    } else {
                        if (pred.toString().contains("java_primitive_char_value")) {
                            theScope.char_op_contracts_count++;
                        }
                    }
                }
            }
        }
        Set<JProgramDeclaration> theProgs = module.getPrograms();
        for (JProgramDeclaration progDecl : module.getPrograms()) {
            String buildFullyTypedProgramName = getFullTypedname(progDecl);
            String fullyQualifiedAndFullyTypedName = TacoConfigurator.getInstance().getClassToCheck() + "_" + buildFullyTypedProgramName;
            if (fullyQualifiedAndFullyTypedName.equals(TacoConfigurator.getInstance().getMethodToCheck())) {
                thePreds = progDecl.getPredsEncodingValueOfArithmeticOperationsInRequiresAndEnsures();
                for (AlloyFormula pred : thePreds){
                    if (pred.toString().contains("java_primitive_integer_value")){
                        theScope.integer_op_contracts_count++;
                    } else {
    ...
    private InputCount countAllInputs() {

        InputCount input_count = new InputCount();
        ...
        for (JDynAlloyModule module : this.src_jdynalloy_modules) {
            if (module.getModuleId().equals(TacoConfigurator.getInstance().getClassToCheck())) {
                for (JProgramDeclaration progDecl : module.getPrograms()) {
                    ...
                    String fullyQualifiedAndFullyTypedName = TacoConfigurator.getInstance().getClassToCheck() + "_" + buildFullyTypedProgramName;
                    if (fullyQualifiedAndFullyTypedName.equals(TacoConfigurator.getInstance().getMethodToCheck())) {
                        for (JVariableDeclaration var : progDecl.getParameters()) {
                            String varName = var.getVariable().getVariableId().getString();
                            if (!varName.equals("thiz") && !varName.equals("throw") && !varName.equals("return")) {
                                if (var.getType().toString().equals("JavaPrimitiveIntegerValue")) {
                                    input_count.integer_input_count++;
                                }
                                if (var.getType().toString().equals("JavaPrimitiveLongValue")) {
                                    input_count.long_input_count++;
                                }
                                if (var.getType().toString().equals("JavaPrimitiveFloatValue")) {
                                    input_count.float_input_count++;
                                }
                                if (var.getType().toString().equals("JavaPrimitiveCharValue")) {
                                    input_count.char_input_count++;
                                }
                            }
                        }
                    }
    ...
    private String getFullTypedname(JProgramDeclaration progDecl) {
        String types =  "(";
        for (JVariableDeclaration jv : progDecl.getParameters()){
            String theParName = jv.getVariable().getVariableId().getString();
            if (!theParName.equals("thiz") && !theParName.equals("throw") && !theParName.equals("return")){
                String jvType = jv.getType().toString();
                if (jvType.equals("JavaPrimitiveIntegerValue"))
                    types += "int";
        ...
        types += ")";
        return progDecl.getProgramId() + types;
    }
