...
public class JError implements IBuiltInModule {

	// public static final JBindingKey NULLPOINTER_EXCEPTION_CONSTRUCTOR_KEY =
	// new JBindingKey(
	// "Ljava/lang/NullPointerException;.()V");

	private static JError instance;

	public static JError getInstance() {
		if (instance == null)
			instance = new JError();
		return instance;
	}
	...
	private JError() {
		...
		JSignature signature = buildClass(signatureIsAbstract, "java_lang_Error",
		...
		JProgramDeclaration constructor = new JProgramDeclaration(false, true, false,
				"java_lang_Error", "Constructor", parameters,
				Collections.<JSpecCase> emptyList(), new JSkip(), new AlloyTyping(), new ArrayList<AlloyFormula>());

		this.module = new JDynAlloyModule("java_lang_Error",
		...
		if (JDynAlloyConfig.getInstance().getNewExceptionsAreLiterals() == true) {
			JSignature literalSingleton = buildLiteralSingleton("java_lang_Error");
