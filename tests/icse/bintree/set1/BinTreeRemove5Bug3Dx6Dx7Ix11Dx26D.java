...
public class BinTreeRemove5Bug3Dx6Dx7Ix11Dx26D {
    ...
    public BinTreeRemove5Bug3Dx6Dx7Ix11Dx26D() {
    }
    ...
    public boolean remove( int element ) { //mutGenLimit 0
        ...
        while (node != null && node.key != element) { //mutGenLimit 0
            if (element > node.key) { //mutGenLimit 1
                ...
                if (this.size > node.key) { //mutGenLimit 1
                    node.parent = node.right; //mutGenLimit 1
                }
            }
        }
        ...
        if (node.left == node) { //mutGenLimit 1
