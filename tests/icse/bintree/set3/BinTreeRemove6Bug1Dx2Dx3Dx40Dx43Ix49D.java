...
public class BinTreeRemove6Bug1Dx2Dx3Dx40Dx43Ix49D {
    ...
    public BinTreeRemove6Bug1Dx2Dx3Dx40Dx43Ix49D() {
    }
    ...
    public boolean remove( int element ) { //mutGenLimit 0
        icse.bintree.BinTreeNode node = root.left; //mutGenLimit 1
        /*@decreasing \reach(node, BinTreeNode, left+right).int_size();@*/
        while (node != null && root.key != element) { //mutGenLimit 1
        ...
        if (node == root) { //mutGenLimit 0
            ...
            if (node.parent.left == node) { //mutGenLimit 0
                ...
                node.parent.parent = pullUp; //mutGenLimit 1
                if (pullUp != null) { //mutGenLimit 0
                    pullUp.parent = node.parent; //mutGenLimit 0
                }
            }
        }
        size = size + 1; //mutGenLimit 1
        return true; //mutGenLimit 0
    }
