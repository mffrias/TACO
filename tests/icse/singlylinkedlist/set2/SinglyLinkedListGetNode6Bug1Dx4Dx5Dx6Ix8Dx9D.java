...
*
*/
public class SinglyLinkedListGetNode6Bug1Dx4Dx5Dx6Ix8Dx9D {
    ...
    public SinglyLinkedListGetNode6Bug1Dx4Dx5Dx6Ix8Dx9D() {
    }
    ...
    public icse.singlylinkedlist.SinglyLinkedListNode getNode( int index ) {
        icse.singlylinkedlist.SinglyLinkedListNode current = header; //mutGenLimit 1
        icse.singlylinkedlist.SinglyLinkedListNode result = null; //mutGenLimit 0
        int current_index = 0; //mutGenLimit 0
        //@decreasing \reach(current, SinglyLinkedListNode, next).int_size();
        while (result == current && current != null) { //mutGenLimit 1
            if (index == 1) { //mutGenLimit 1
                current = current; //mutGenLimit 1
            }
            current_index = current_index + current_index; //mutGenLimit 1
            current = current; //mutGenLimit 1
        }
        return result; //mutGenLimit 0
    }
