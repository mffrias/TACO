...
public class BinTreeContainsBug3x6x4Dx7D {
    ...
    @*/
    public /*@nullable@*/pldi.bintree.BinTreeNode root;

    public int size;

    public BinTreeContainsBug3x6x4Dx7D() {
    }
    ...
    public boolean contains( int k ) {
        pldi.bintree.BinTreeNode current = root;
    ...
    public boolean insert( int k ) {
        pldi.bintree.BinTreeNode y = null;
        pldi.bintree.BinTreeNode x = root;
        while (x != null) {
            y = x;
            if (k < x.key) {
                x = x.left;
        ...
        x = new pldi.bintree.BinTreeNode();
        x.key = k;
        if (y == null) {
            root = x;
        } else {
    ...
    public boolean remove( int element ) {
        pldi.bintree.BinTreeNode node = root;
        ...
        if (node == null) {
            ...
            if (node.left != null && node.right != null) {
                pldi.bintree.BinTreeNode predecessor = node.left;
                if (predecessor != null) {
        ...
        pldi.bintree.BinTreeNode pullUp;
        if (node.left == null) {
            pullUp = node.right;
        } else {
            pullUp = node.left;
